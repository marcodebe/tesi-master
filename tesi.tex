...
Xtables per definire l'intero firewall.
...
interno.  Il firewall esterno regola il traffico tra internet, DMZ e
reti locali; quello interno regole il traffico tra le reti locali e l'esterno.

\begin{figure}
\begin{center}
      \includegraphics[width=.8\linewidth]{net.svg.png}
...
decresce mai, ad esempio al momento sono presenti oltre 23 mila indirizzi ethernet.
...
Inizialmente, infatti, per ogni indirizzo era prevista una regola\footnote{Di
tipo {\em REJECT} per le blacklist e di tipo {\em RETURN} per la gestione del
wifi free.}, ciò significa che prima di poter classificare ogni singolo
pacchetto \`e necessario scorrere una per una migliaia se non decine di
migliaia di regole (ovviamente non subiscono questa verifica i pacchetti
relativi a connessioni gi\`a stabilite).  Questo approccio, basato su una regola
per ogni indirizzo, risulta essere non efficiente, superata una certa
...
insiemi, ma uno strumento esterno, ipset, fornisce questa funzionalità
...
monitor da console e rimanere in ascolto:
\begin{lstlisting}[style=customb]
# nft monitor
\end{lstlisting}
...
\begin{lstlisting}[style=customc,firstnumber=20]
        ip daddr $WIFINET limit rate over 6 mbytes/second \
                     burst 10 mbytes/second drop
\end{lstlisting}
...
\begin{lstlisting}[style=customc,firstnumber=20]
        ip daddr $WIFINET meter rate-meter { 
             ip saddr . ip daddr limit rate over 3 mbytes/second
             burst 5 mbytes/second } counter drop
\end{lstlisting}
In questo caso le limitazioni sono per coppie di indirizzi sorgente e destinazione,
il punto concatena i due indirizzi generado la chiave dell'elemento nel meter.
...
\begin{center}
  ...
  \begin{table}[ht]
    \centering % used for centering table
      \begin{tabular}{@{}llcc@{}}
     \toprule
     {\bf Configurazione} & {\bf iperf} & {\bf wget 4GB} & {\bf }\\ \midrule
         routing  & 32.6                & 4.5  & \\
         mark     & 30.3                & 4.4  & \\ [0.5ex]
         mark-full& 30.2                & 4.4  & \\ [0.5ex]
          &           &      & \\ \bottomrule
      \end{tabular}  
